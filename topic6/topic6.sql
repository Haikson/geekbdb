...
/*
***** media_type_id = (SELECT id FROM media_types WHERE name LIKE 'photo') ****
Если не правильно спроектирована таблица media_types, то есть вероятность дублирования и при таком запросе получим больше одной строки с именем 'photo'
Стоит вырабатывать привычку в данном случае использовать
1. media_type_id = (SELECT id FROM media_types WHERE name LIKE 'photo' LIMIT 1)
2. media_type_id IN (SELECT id FROM media_types WHERE name LIKE 'photo')

Естественно LIKE следует заменить на = либо
*/
